...
static bool run_with_sudo(const std::string& script_content) {
    ...
    std::string tmp = std::string(home) + "/.ventctl_tmp.sh";
    {
...
bool daemon_cmd_install(const std::vector<std::string>& /*args*/) {
    ...
    std::cout << "Installing ventd daemon (requires admin privileges)...\n";

    // Get path of ventd binary
    char self_path[4096];
    uint32_t self_size = sizeof(self_path);
    std::string ventd_src = "/usr/local/bin/ventd";

    // Try to find ventd next to ventctl
    if (_NSGetExecutablePath(self_path, &self_size) == 0) {
        std::string self(self_path);
        auto pos = self.rfind('/');
        if (pos != std::string::npos) {
            std::string dir = self.substr(0, pos + 1);
            std::string sibling = dir + "ventd";
    ...
    std::string script =
        ...
        "\n"
        "# Copy binary\n"
        "cp -f '" + ventd_src + "' /usr/local/bin/ventd 2>/dev/null || {\n"
        "    echo 'ventd binary not found at " + ventd_src + "'\n"
        "    exit 1\n"
        "}\n"
        "chmod 755 /usr/local/bin/ventd\n"
        "chown root:wheel /usr/local/bin/ventd 2>/dev/null || true\n"
        "\n"
        "# Create plist\n"
        ...
        "\n"
        "# Load daemon\n"
        "launchctl bootstrap system /Library/LaunchDaemons/com.vent.daemon.plist 2>/dev/null || "
        "launchctl load /Library/LaunchDaemons/com.vent.daemon.plist\n"
    ...
    std::cerr << "Daemon may not have started. Check /var/log/ventd.log\n";
    return false;
}

bool daemon_cmd_uninstall(const std::vector<std::string>& /*args*/) {
    std::cout << "Uninstalling ventd daemon (requires admin privileges)...\n";

    std::string script =
        "#!/bin/bash\n"
        "launchctl bootout system/com.vent.daemon 2>/dev/null || "
        "launchctl unload /Library/LaunchDaemons/com.vent.daemon.plist 2>/dev/null || true\n"
        "killall ventd 2>/dev/null || true\n"
        "rm -f '" + std::string(DAEMON_SOCKET_PATH) + "'\n"
        "rm -f '" + std::string(DAEMON_PID_PATH) + "'\n"
        "rm -f /Library/LaunchDaemons/com.vent.daemon.plist\n"
        "rm -f /var/log/ventd.log /var/log/ventd.err\n"
